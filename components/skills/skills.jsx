...
const Ability = dynamic(()=>import("../ability/Ability"))
...
const Skills = () => {
  return (
    <div
      className="pb-4 pt-32 px-2 lg:px-10 section min-h-screen relative"
      id="skills"
    >
      <div
        className="mb-20"
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 max-w-[1200px] mx-auto w-full mt-5 gap-4">
        {theSkills.map((el) => (
          <Suspense     key={el.name} fallback={<Loader />}> 
          <Ability
      ...
      <motion.h3
        ...
        className="uppercase text-gray-600 mt-32 relative"
      >
      ...
      <div className="max-w-[1200px] w-full mx-auto gap-6 sm:px-6 mt-14 justify-center flex flex-wrap ">
        {certificates.map((el) => (
          <Suspense key={el.id} fallback={<Loader />}>
            <Certificate data={el} />
          </Suspense>
        ))}
      </div>
